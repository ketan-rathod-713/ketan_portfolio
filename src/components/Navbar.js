...
const Navbar = () => {
  ...
  return (
    <div className='shadow-md w-full fixed top-0 left-0 relative z-30'>
      ...
      <ul className={`${open ? theme==='dark' ? 'bg-gray-700' :'bg-gray-600 text-white': theme==='dark' ? 'bg-gray-700' :'bg-white'} md:flex md:items-center md:pb-0 pb-12 absolute md:static md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? 'top-20 ':'top-[-490px]'}`}>
        {
          Links.map((link)=>(
            <li key={link.name} className='md:ml-8 text-xl md:my-0 my-7'>
            <Link to={link.link} className=' hover:text-gray-300 duration-500'>{link.name}</Link>
